    ...
    if not url:
        ...
        with st.spinner('Generating...'):
            ...
            with zipfile.ZipFile(zip_buffer, "w") as zf:
                ...
                # Keywords aur description ko string mein convert kar rahe hain
                seo_text = f"Keywords: {', '.join(seo_data.get('keywords', []))}\n\nDescription: {seo_data.get('description', '')}"
                
                pdf = FPDF()
                pdf.add_page()
                pdf.set_font("Arial", size=12)
                pdf.cell(200, 10, txt="SEO Strategy", ln=True)
                # Ab hum sirf plain string pass kar rahe hain
